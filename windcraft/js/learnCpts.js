                ...
                callback: function () {
                    ...
                    d3.selectAll("input.csv-header")[0].forEach(function(header) {
                    	...
                    	if(isEmptyString(header.value)) {
							bootbox.dialog({
							  message: "Empty rung names are not allowed.",
							  buttons: {
							    main: {
							      label: "OK",
                    ...
                    if(_.uniq(headers).length !== headers.length) {
						bootbox.dialog({
						  message: "Duplicate rung names are not allowed.",
						  buttons: {
						    main: {
						      label: "OK",
						      className: "btn-bayes-short",
...
var createNodes = function(fdata) {
	...
	var text = warningDiv.html() + " Learning the structure of an attack ladder is a computationally complex problem. An attempt to learn the structure of an attack ladder bigger than 10 rungs might crash your browser."				  		  
...
var checkNoObservedValues = function() {
	...
	if(!flag) {
		bootbox.dialog({
		  message:"Less than 2 observed values for a rung are not allowed.",
		  buttons: {
		    main: {
		      label: "OK",
		      className: "btn-bayes-short",
