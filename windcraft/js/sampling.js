...
var displaySamples = function(samples, noSample, fSample) {
	...
	var filenameText = ""; //fileHeader.html() + "<hr><h5>Previously Saved Report: "+ docxFilename + "</h5><hr>"
	...
	if (typeof previousSystemResilienceComputation === 'undefined' || previousSystemResilienceComputation === null) {
		resilienceText = resilienceHeader.html() + 
			...
			"It reflects the percentage of samples where any end rung of the attack ladder is compromised.</p><hr>";
	} else {
		resilienceText = resilienceHeader.html() + 
			"<h4>Overall System Resilience: " + systemResilienceComputation + " </h4><hr>" +
...
var samplingSettings = function(){
	...
	mitreAttack.append("span")
			  .attr("class", "sr-only")
			  .text("");
	var text = mitreAttack.html() + "Loading "+  "<a href="+desiredLink+" target=_blank>"+desiredText+"</a>"+ 
			  " in JSON format via Load/Save may be helpful in setting specific rungs."
